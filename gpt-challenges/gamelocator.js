...
const verificaJogoExistencia = (jogo) => {
  if (
    locadora.jogosDisponiveis.some((j) => j.id === jogo.id) ||
    locadora.jogosAlugados.some((j) => j.id === jogo.id)
  ) {
    return { ok: true };
  }
...
const alugaJogo = (jogo) => {
  ...
  if (locadora.jogosDisponiveis.some((j) => j.id === jogo.id)) {
    locadora.jogosDisponiveis = locadora.jogosDisponiveis.filter(
      (j) => j.id !== jogo.id,
    );
...
const devolveJogo = (jogo) => {
  ...
  if (locadora.jogosAlugados.some((j) => j.id === jogo.id)) {
    locadora.jogosAlugados = locadora.jogosAlugados.filter(
      (j) => j.id !== jogo.id,
    );
    locadora.jogosDisponiveis.push(jogo);
