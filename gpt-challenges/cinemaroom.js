...
const cancelarIngresso = (numeroAssento) => {
  ...
  if (cinema.assentosOcupados.includes(numeroAssento)) {
    cinema.assentosOcupados = cinema.assentosOcupados.filter(
      (assento) => assento !== numeroAssento,
    ); // Se o número do assento for diferente do número que quer ser cancelado, vai retornar true, formando um novo array pra dentro de assentosOcupados
